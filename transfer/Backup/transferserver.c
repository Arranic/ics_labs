...
struct msg
{
    uint32_t length;                         // max length of 4096 - 8 == 4088
    char buffer[BUFSIZE - sizeof(uint32_t)]; // buffer[4088]
};
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        */
        ...
        // int remainder = 0;           // used to track how many bytes didn't get sent
        memset(fileBuf, 0, BUFSIZE); // zero out the buffer allocated with BUFSIZE bytes

        // struct msg message;                                     // should be exactly 4096 bytes in size
        // memset(&message.buffer, 0, BUFSIZE - sizeof(uint32_t)); // zero out the message buffer

        send(clientSock, &netFileSize, sizeof(netFileSize), 0); // send the size of the file
        dataSent = 0;
        while (dataSent < fileSize) // read the data until the end of the file 4088 bytes at a time
        {
            dataRead = read(filefd, fileBuf, BUFSIZE);
            if (dataRead < 0)
            {
                fprintf(stderr, "File read error.\n");
                exit(EXIT_FAILURE);
            }
            ssize_t numBytesToSend = dataRead;
            ...
            // message.length = dataRead;                        // the length of the packet data will be the amount of data we read in
            // memcpy(&message.buffer, fileBuf, message.length); // copy message.length amount of data from fileBuf to the message.buffer

            // dataSent = send(clientSock, &message, message.length + sizeof(uint32_t), 0); // send the data
            // remainder = dataRead - dataSent;
            // while (remainder > 0) // if we didn't send all the data that we read in, loop until we send it all
            // {
            //     // printf("remainder: %d\n", remainder);
            //     message.length = remainder;
            //     dataSent += send(clientSock, &message, message.length + sizeof(uint32_t), 0);
            //     remainder = dataRead - dataSent;
            // }
            // memset(fileBuf, 0, BUFSIZE);                            // reset buffer to read in more data
            // memset(&message.buffer, 0, BUFSIZE - sizeof(uint32_t)); // zero out the message buffer to send the additional data
            printf("dataSent %lu\n", dataSent);
            totalPackets++;
        }

        // Theoretically, at this point, the entire 100-byte buffer should already have been sent
        // so we'll clear the buffer to read in more data
